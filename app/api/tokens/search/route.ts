...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedResults = searchResults.map(token => {
      ...
      let currentPrice = 0;
      if (token.pool_stats?.hbar_reserves) {
        // Simple price calculation - you may want to use the actual bonding curve formula
        const hbarReserves = Number(token.pool_stats.hbar_reserves) / 1e18; // Convert from 18-decimal units
        const virtualTokenReserves = 1000000000; // 1B tokens virtual reserves
        currentPrice = hbarReserves / virtualTokenReserves;
      }
