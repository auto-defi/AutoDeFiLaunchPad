...
function InfoTicker({ token }: { token: TokenDetail }) {
  const tickerItems = [
    {
      label: "Volume 24h",
      value: `${formatCompactNumber(Number(token.pool_stats?.total_volume || 0) / 1e18, 2)} HBAR`,
      icon: "📊"
    },
    {
      label: "Total Trades",
    ...
    {
      label: "Progress",
      value: `${((Number(token.pool_stats?.hbar_reserves || 0) / Number(token.pool_stats?.graduation_threshold || 1)) * 100).toFixed(1)}%`,
      icon: "🎯"
    },
    {
      label: "Creator",
...
const account = useMemo(() => ({ address: evmAccount || null }), [evmAccount]);

  const GRADUATION_THRESHOLD = 21500;
  const VIRTUAL_HBAR_RESERVES = 28.24;

  const [token, setToken] = useState<any>(null);
  const [selectedToken, setSelectedToken] = useState<TokenInfo | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [evmMetrics, setEvmMetrics] = useState<{ currentPrice: number; priceChange24h: number; marketCap: number; volume24hUsd: number } | null>(null);
  ...
  const getUserTokenBalance = useCallback(async (tokenAddress: string) => {
    // Hedera EVM: TODO - query ERC-20 balance via ethers/signer if needed
    return 0;
  }, []);
    ...
    try {
      ...
      const tokenPromises = apiTokens.map(async (token: any) => {
        try {
          ...
          const hbarReserves = token.pool_stats ? Number(token.pool_stats.hbar_reserves) / 100000000 : 0;
      ...
      const resolvedTokens = (await Promise.all(tokenPromises))
        ...
        .sort((a, b) => b.hbar_reserves - a.hbar_reserves);
  ...
  const calculateTokensOut = (hbarIn: number, hbarReserves: number, tokenSupply: number): number => {
    const x = hbarReserves + VIRTUAL_HBAR_RESERVES;
    const y = tokenSupply;
    return (y * hbarIn) / (x + hbarIn);
  };

  const calculateHbarOut = (tokensIn: number, hbarReserves: number, tokenSupply: number): number => {
    const x = hbarReserves + VIRTUAL_HBAR_RESERVES;
    const y = tokenSupply;
  ...
  const estimatedTokens = useMemo(() => {
    if (!selectedToken || !hbarAmount || isNaN(Number(hbarAmount))) return 0;
    return calculateTokensOut(
      Number(hbarAmount),
      selectedToken.hbar_reserves,
      selectedToken.token_supply
    );
  }, [hbarAmount, selectedToken]);

  const estimatedHbar = useMemo(() => {
    if (!selectedToken || !tokenAmount || isNaN(Number(tokenAmount))) return 0;
    return calculateHbarOut(
      Number(tokenAmount),
      selectedToken.hbar_reserves,
      selectedToken.token_supply
    );
  ...
  const fetchTokenDetail = async () => {
    try {
      setLoading(true);
      // Fetch EVM metrics in parallel
      void (async () => {
        try {
          const mres = await fetch(`/api/indexer-evm/metrics?tokens=${address}`);
          const mjson = await mres.json();
          const row = Array.isArray(mjson.data) ? mjson.data[0] : null;
          if (row) setEvmMetrics({
      ...
      if (data.success) {
        ...
        if (data.data.pool_stats) {
          const hbarReservesVal = Number(data.data.pool_stats.hbar_reserves);
          const threshold = Number(data.data.pool_stats.graduation_threshold);
          const progress = (hbarReservesVal / threshold) * 100;

          console.log(`📈 Graduation Progress Calculation:`, {
            hbar_reserves_units: hbarReservesVal,
            hbar_reserves_hbar: (hbarReservesVal / 1e18).toFixed(2) + ' HBAR',
            graduation_threshold_units: threshold,
            graduation_threshold_hbar: (threshold / 1e18).toFixed(0) + ' HBAR',
  ...
  const currentPrice = evmMetrics?.currentPrice ?? (token.pool_stats ?
    Number(token.pool_stats.hbar_reserves) / 1e18 / 1000000000 : 0);

  const graduationProgress = token.pool_stats ?
    (Number(token.pool_stats.hbar_reserves) / Number(token.pool_stats.graduation_threshold)) * 100 : 0;

  const handleBuyTokens = async () => {
    toast.info("Buy from coin detail is not available yet on Hedera EVM", {
      description: "Please use the main Trading page for now.",
      duration: 5000,
    });
  };

  const handleSellTokens = async () => {
    toast.info("Sell from coin detail is not available yet on Hedera EVM", {
      description: "Please use the main Trading page for now.",
      duration: 5000,
    });
  };
  ...
  const handleDEXSwapBuy = async () => {
    toast.info("DEX swap (buy) is not available yet on Hedera EVM", {
      description: "This will be enabled after DEX integration.",
      duration: 5000,
    });
  };

  const handleDEXSwapSell = async () => {
    toast.info("DEX swap (sell) is not available yet on Hedera EVM", {
      description: "This will be enabled after DEX integration.",
      duration: 5000,
    });
  };
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-7xl p-4 pt-8 relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <EnhancedStatsCard
            title="24h Volume"
            value={`${formatCompactNumber(evmMetrics?.volume24hUsd ?? (token.volume_24h / 1e18), 2)} ${evmMetrics ? 'USD' : 'HBAR'}`}
            subtitle="traded"
                  ...
                  <Card className="bg-gradient-to-br from-black/40 via-purple-900/20 to-black/40 border border-white/20 backdrop-blur-xl hover:border-white/40 transition-all duration-500 relative overflow-hidden">
                    ...
                    <CardContent className="space-y-6 relative z-10">
                      {[
                        {
                          label: "HBAR Reserves",
                          value: `${formatCompactNumber(Number(token.pool_stats?.hbar_reserves || 0) / 1e18, 2)} HBAR`,
                          icon: "💰"
                        },
                        {
                          label: "Total Volume",
                          ...
                          <div className="p-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                              <div className="text-center p-3 rounded-lg bg-muted/20">
                                <p className="text-2xl font-bold text-primary">{formatCompactNumber(selectedToken?.hbar_reserves || 0, 2)}</p>
                                <p className="text-sm text-muted-foreground">HBAR Reserves</p>
                              </div>
                              <div className="text-center p-3 rounded-lg bg-muted/20">
                            ...
                            <div className="mb-6">
                              <div className="flex justify-between text-sm mb-2">
                                ...
                                <span>{formatCompactNumber(GRADUATION_THRESHOLD, 1)} HBAR Target</span>
                              </div>
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab}>
                              ...
                              <TabsContent value="buy" className="space-y-4 mt-6">
                                <div className="space-y-4">
                                  <div>
                                    <Label htmlFor="hbar-amount">HBAR Amount</Label>
                                    <Input
                                      id="hbar-amount"
                                      type="number"
                                      placeholder="0.0"
                                  ...
                                  <Button
                                    ...
                                    disabled={!hbarAmount || selectedToken?.is_graduated || !account}
                              ...
                              <TabsContent value="sell" className="space-y-4 mt-6">
                                <div className="space-y-4">
                                  ...
                                  <div>
                                    <Label htmlFor="hbar-estimate">You'll receive (estimated)</Label>
                                    <Input
                                      id="hbar-estimate"
                                      value={estimatedHbar.toFixed(6)}
                                      readOnly
                                      className="bg-muted/10"
                                    />
                                    <p className="text-xs text-muted-foreground mt-1">HBAR</p>
                                  </div>
                              ...
                              <div className="p-6 rounded-lg bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/30 mt-6">
                                ...
                                <Tabs value={activeTab} onValueChange={setActiveTab}>
                                  ...
                                  <TabsContent value="buy" className="space-y-4 mt-6">
                                    <div className="space-y-4">
                                      <div>
                                        <Label htmlFor="hbar-amount-dex">HBAR Amount</Label>
                                        <Input
                                          id="hbar-amount-dex"
                                          type="number"
                                          placeholder="0.0"
                                  ...
                                  <TabsContent value="sell" className="space-y-4 mt-6">
                                    <div className="space-y-4">
                                      ...
                                      <div>
                                        <Label htmlFor="hbar-estimate-dex">You'll receive (estimated)</Label>
                                        <Input
                                          id="hbar-estimate-dex"
                                          value={estimatedHbar.toFixed(6)}
                                          readOnly
                                          className="bg-muted/10"
                                        />
                                        <p className="text-xs text-muted-foreground mt-1">HBAR (via DEX)</p>
                                      </div>
                                      ...
                                      >
                                        {`Swap ${selectedToken?.symbol} → HBAR`}
                                      </Button>
                                    </div>
                                  </TabsContent>
                                </Tabs>
                        ...
                        token.trades.map((trade: any, index: number) => (
                          ...
                          >
                            <div className="flex items-center space-x-4">
                              ...
                              <div>
                                <p className="text-white font-bold text-lg">
                                  {formatCompactNumber(Number(trade.token_amount) / Math.pow(10, token.decimals), 2)} {token.symbol}
                                </p>
                                <motion.p
                            ...
                            <div className="text-right">
                              ...
                              >
                                {formatCompactNumber(Number(trade.hbar_amount) / 1e18, 2)} HBAR
                              </motion.p>
                  ...
                  <CardContent className="space-y-6 relative z-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {[
                        ...
                        { label: "Max Supply", value: token.max_supply ? formatCompactNumber(Number(token.max_supply) / Math.pow(10, token.decimals), 0) : "1B", icon: "💰" }
                      ].map((info, index) => (
                        <motion.div
                          key={info.label}
                          initial={{ scale: 0.8, opacity: 0 }}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                        {[
                          ...
                          { label: "Total Volume", value: `${formatCompactNumber(Number(token.pool_stats?.total_volume || 0) / 1e18, 2)} HBAR` },
                          { label: "Mint Fee", value: `${token.mint_fee_per_unit} HBAR` }
                        ].map((stat, index) => (
                          <motion.div
                            key={stat.label}
                            whileHover={{ scale: 1.05, y: -2 }}
