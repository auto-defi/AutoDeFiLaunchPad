...
export default function LaunchTokenForm() {
  ...
  const { account, signer, connected, connect } = useEVMWallet() as any;
  ...
  async function handleDeploy() {
    try {
      ...
      if (!FACTORY_ADDR) {
        toast.error("Factory address not set", { description: "Set NEXT_PUBLIC_FACTORY_ADDRESS in your env" });
        return;
      }
      if (!name.trim()) {
      ...
      const initialBuyWei = initialBuyAmount && parseFloat(initialBuyAmount) > 0
        ? ethers.parseEther(initialBuyAmount)
        : ethers.toBigInt(0);
      const valueHex = initialBuyWei > ethers.toBigInt(0)
        ? ethers.toQuantity(initialBuyWei)
        : undefined;

      const factory = new ethers.Contract(FACTORY_ADDR, (factoryAbi as any).abi || (factoryAbi as any), signer);
      ...
      // Try to parse TokenCreated event to surface token/pool addresses
      try {
        const iface = new ethers.Interface(((factoryAbi as any).abi || (factoryAbi as any)));
        let tokenAddr = ""; let poolAddr = "";
        ...
        if (tokenAddr && poolAddr) {
          toast.success("🎉 Token created!", {
            description: (
              <div className="space-y-1">
                <div>Token: {String(tokenAddr)}</div>
                <div>Pool: {String(poolAddr)}</div>
                <button
                  onClick={() => window.open(`https://hashscan.io/testnet/contract/${poolAddr}`, '_blank')}
                  className="flex items-center gap-1 text-blue-400 hover:text-blue-300"
                >
                  <ExternalLink size={12} /> View Pool on HashScan
                </button>
              </div>
            ),
            duration: 10000
          ...
          toast.success("🎉 Token created!", {
            description: (
              <div className="space-y-1">
                <div>Tx: {String(txHash).slice(0, 10)}...</div>
                <button
                  onClick={() => window.open(`https://hashscan.io/testnet/transaction/${txHash}`, '_blank')}
                  className="flex items-center gap-1 text-blue-400 hover:text-blue-300"
                >
                  <ExternalLink size={12} /> View Tx on HashScan
                </button>
              </div>
            )
          });
        ...
        console.warn("Could not parse logs:", parseErr);
        toast.success("🎉 Token created!");
      }
      ...
      const isUserRejection = (e?.code === 4001) || /reject|denied|cancel/i.test(msg);
      if (isUserRejection) {
        toast.info("Transaction cancelled by user");
        return;
      }
      console.error("Deployment error:", e);
      toast.error("Failed to create token", { description: msg });
    } finally {
      setSubmitting(false);
    }
  }
                ...
                >
                  ...
                  <div className="text-xs text-blue-300/80 space-y-2">
                    ...
                    <div className="break-all font-mono">
                      Factory: {FACTORY_ADDR ? `${FACTORY_ADDR.substring(0, 10)}...` : "Not set"}
                    </div>
                  </div>
                </motion.div>
