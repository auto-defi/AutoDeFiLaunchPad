    ...
    try {
      ...
      const tokenPromises = apiTokens.map(async (token: any) => {
        try {
          ...
          const hbarReserves = token.pool_stats ? Number(token.pool_stats.hbar_reserves) : 0;
...
function TokenCard({ token, index, graduationThreshold }: TokenCardProps) {
  const graduationProgress = Math.min((token.hbar_reserves / graduationThreshold) * 100, 100);
  ...
  const marketCap = `$${(token.hbar_reserves * 10).toFixed(1)}K`; // Mock market cap
